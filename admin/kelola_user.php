...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tambah_user'])) {
    ...
    $roleUser     = mysqli_real_escape_string($conn, $_POST['role']);
    $status       = mysqli_real_escape_string($conn, $_POST['status']);
    $password_hash= password_hash($password, PASSWORD_DEFAULT);

    $sql = "INSERT INTO users (username, nama_lengkap, email, password, role, status) 
            VALUES ('$username', '$nama_lengkap', '$email', '$password_hash', '$roleUser', '$status')";
    if (mysqli_query($conn, $sql)) {
        $message = '<div class="notification success">User berhasil ditambahkan!</div>';
    } else {
        $message = '<div class="notification error">Gagal menambah user!<br>' . mysqli_error($conn) . '</div>';
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_user'])) {
    ...
    if (!empty($_POST['password'])) {
        ...
        $sql = "UPDATE users SET 
                username='$username', nama_lengkap='$nama_lengkap', email='$email',
                password='$password_hash', role='$roleUser', status='$status'
                WHERE id=$id";
    } else {
        $sql = "UPDATE users SET 
                username='$username', nama_lengkap='$nama_lengkap', email='$email',
    ...
    if (mysqli_query($conn, $sql)) {
        $message = '<div class="notification success">User berhasil diperbarui!</div>';
    } else {
        $message = '<div class="notification error">Gagal memperbarui user!<br>' . mysqli_error($conn) . '</div>';
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['hapus_user'])) {
    ...
    if (mysqli_query($conn, $sql)) {
        ...
        $message = '<div class="notification error">Gagal menghapus user!<br>' . mysqli_error($conn) . '</div>';
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tolak_user'])) {
    ...
    $message = '<div class="notification error">User ditolak dan dihapus!</div>';
}
?>
<!DOCTYPE html>
